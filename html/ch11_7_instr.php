...
<head>
<title>EMBER: Chapter 11 - Investigating the inositol phosphatase family: Instructions Step 7</title>
...
?> 
<p>Here we will search the 
 Pfam database and also attempt to build an HMM. </p>
<p>i) Examine the
 <?php $Url_id="148"; $Link_naam="Pfam"; include ('get_link.php'); ?> database. Go to <i>Browse PFAM</i> and display the <i>Domain Organisation</i> of
a SWISS-PROT or TrEMBL sequence (<em>e.g.</em>, F16P_HUMAN).</p>
<p>ii) Moving the mouse over a domain provides a description of the domain. Click
 on any of the domains and retrieve the Pfam entry. From the Pfam entry, retrieve
 an alignment: go to the <i>Alignment</i> option and select Jalview as format
 and retrieve the alignment. Explore some of the options of Jalview (<em>e.g.,</em> make
 a tree). There are several <i>Database References</i> in the Pfam entry - explore
 as many of them as possible. </p>
<p>iii) Upload the alignment generated in the previous step into
  <?php $Url_id="149"; $Link_naam="hmmbuild"; include ('get_link.php'); ?>.
  Run the program, and inspect the results. </p>
<p><em>Reflections...</em></p>
	<ul>
 <li><em>In what ways do 
  profiles and HMMs differ? In what ways are they similar? </em></li>
 <li><em>What is the advantage 
  of using profiles or HMMs over, say, regular expressions?</em></li>
</ul>
