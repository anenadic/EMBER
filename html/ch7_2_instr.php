...
<p>Here we will investigate another homology modelling service.</p>
i) Paste your sequence into the <?php $Url_id="121"; $Link_naam="Geno3D"; include ('get_link.php'); ?> input
form, which will run PSI-BLAST. From the results, select the most significant
hits as templates (no more than 10). <em>NOTE: If you get an error message, reduce
the number of PSI-BLAST iterations. </em>
<p>ii) A schematic diagram illustrating alignments between your query and the selected 
 templates is returned on the <em>Template Validation</em> page. If satisfied 
 with the result, launch Geno3D, remembering to provide your email address. </p>
<p><em>Reflections...</em></p>
<ul>
 <li><em><i>How many matches 
  are statistically significant? </i></em></li>
 <li><em>Are the matches plausible - 
  are the proteins related? </em></li>
 <li><em><i>Did you reject any sequence 
  matches. If so why?</i></em></li>
 <li><em><i>How similar are the model and 
  template structures? </i></em></li>
 <li><em><i>Did Geno3D and SWISS-MODEL
  use the same template structures to build their models? If there 
  are differences, why might this be so?</i></em></li>
</ul>
